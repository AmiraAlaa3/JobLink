        ...
        class="border p-5">
            ...
            <div class="form-group  mb-3">
                ...
                <input type="text" name="skills_required" class="form-control"
                    value="{{ old('skills_required', $job->skills_required ?? '') }}" required>
            </div>
            ...
            <div class="form-group  mb-3">
                ...
                <input type="text" name="salary_range" class="form-control"
                    value="{{ old('salary_range', $job->salary_range ?? '') }}" required>
            </div>
                ...
                <select name="location_id" class="form-control" required>
                    @foreach ($locations as $location)
                        <option value="{{ $location->id }}"
                            {{ isset($job) && $job->location_id == $location->id ? 'selected' : '' }}>{{ $location->name }}
                        </option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="form-group  mb-3">
                ...
                <select name="work_type" class="form-control" required>
                    <option value="remote" {{ isset($job) && $job->work_type == 'remote' ? 'selected' : '' }}>Remote
                    </option>
                    <option value="on-site" {{ isset($job) && $job->work_type == 'on-site' ? 'selected' : '' }}>On-Site
                    </option>
                    <option value="hybrid" {{ isset($job) && $job->work_type == 'hybrid' ? 'selected' : '' }}>Hybrid
                    </option>
                </select>
            </div>
